...
`update_method_source` (rewrite a method body in C#; the declared method name must match and its full signature, static/instance, parameter count and types, and return type, must match the target, checked before any write). Only imperative bodies are supported (no lambdas, async, iterators, or local functions); the body may use `this` and the target's own private members for non-generic, top-level classes, sealed ones included:
