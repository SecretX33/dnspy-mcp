...
`update_method_source` limits: the declared method name must match the target and the full signature must line up (static/instance, parameter count and types, return type). Only imperative bodies are supported (no lambdas, async, iterators, or local functions, since these need generated helper types that do not exist in the target). The body can use `this` and the target's own private members when the target is a non-generic, top-level class, sealed classes included (it is compiled as a subclass of a publicized, unsealed view of the target); value types, delegates, static classes, and generic or nested targets fall back to a wrapper without this or private access, and the result says which path was taken. Ambiguous overload calls and composed types over the target's own types are rejected rather than mis-bound.
